...
- 定时：node-cron（可选）
- 邮件通知：nodemailer（SMTP，管线 run() 末尾发每日摘要：只推 T0/T1 信源，HTML 卡片 + 纯文本双格式，事件+中文摘要+彩色徽章；空结果照发）
...
- **Windows 定时任务 `ai-news-monitor-daily`（Phase10）**：`npm run ops:schedule` 注册每日 08:00 跑 `scripts/run-pipeline.js`（chdir 仓库根 → 幂等 `docker start crawl4ai` → `node src/index.js` → 追加 `logs/pipeline-YYYY-MM-DD.log`）。任务以当前用户**登录时**运行（无需管理员），机器休眠/未登录不触发；卸载 `npm run ops:unschedule`。前端已部署 Vercel（`ai-news-monitor-silk.vercel.app`），本地兜底 `npm run serve` **B1 已修（2026-08-09）**：日志收尾 `child.stdout.pipe(log, { end: false })`——根因是 pipe 默认 `end:true`，stdout 的 `'end'` 先于子进程 `'exit'` 把 log 流提前关闭，`finishLog` 的 write 落到已关闭流被静默丢弃（exit 行丢失）；修复为两处 `{ end: false }` + finishLog write→end 回调链落盘后再 `process.exit`，`=== <ts> pipeline exited (code=0) ===` 结尾行稳定存在
