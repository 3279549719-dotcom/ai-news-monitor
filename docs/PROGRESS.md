...
- [ ] 重构遗留决策（2026-08-04，/simplify 跳过待定）：① 删 blog 管线（scraper/reader/summarizeArticle）— 注意 `claude-blog` 关键词仍启用，删除前需先下线该关键词；② X 通道策略表 CHANNEL_POLICY 集中 isXUrl 判定；③ 抓取有界并发（现串行，B-003 曾因并发压垮 Supabase 连接池）；④ BoardView 改读 category_schema 数据驱动（现前端硬编码 MU_BOARDS）；⑤ relativeTime 换 date-fns（会改变用户可见中文文案）
