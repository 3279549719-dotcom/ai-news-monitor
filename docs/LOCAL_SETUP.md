...
管线 `run()` 末尾自动发送每日摘要邮件（精简列表，按关键词分组；空结果也发「今日无新增」文案）。可选功能，未配置时自动跳过；发送失败不影响管线退出码。

| 变量 | 说明 |
|---|---|
| `SMTP_HOST` | SMTP 服务器（QQ 邮箱 `smtp.qq.com`、163 邮箱 `smtp.163.com`） |
| `SMTP_PORT` | 端口（默认 465，SSL） |
| `SMTP_SECURE` | 是否 SSL 加密（默认启用；设 `0` 关闭） |
| `EMAIL_USER` | 发件邮箱账号 |
| `EMAIL_AUTH_CODE` | **SMTP 授权码**（非登录密码） |
| `RECEIVER_EMAIL` | 收件邮箱 |

- 授权码获取：QQ 邮箱「设置 → 账号 → 开启 SMTP 服务 → 生成授权码」（163 邮箱类似），将生成的授权码填入 `EMAIL_AUTH_CODE`
- `EMAIL_ENABLED=0` 可整体禁用邮件功能（默认启用）
- 每日 08:00 定时管线跑完后自动发送；即使当日无新增内容也会照常发送
