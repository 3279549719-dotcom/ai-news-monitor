...
- 框架与语言：Node.js CommonJS，无 TypeScript
- 包管理器：npm
- 数据存储：Supabase (PostgreSQL) — keywords 表 + articles 表
...
| store.js | 数据访问：loadKeywords、filterNewItems（RPC）、saveArticles | db.js |
| scraper.js | blog 类型：抓取指定博客文章列表，支持传入 URL | axios, cheerio |
| reader.js | blog 类型：读取单篇文章正文，截4000字 | axios, cheerio |
| search.js | search 类型：Google News RSS + HackerNews Algolia 并行搜索 | axios, cheerio |
| ai.js | summarizeArticle（blog全文→摘要）, analyzeResult（标题+snippet→相关度+摘要）| openai SDK |
...
for each keyword (顺序处理):
  ...
  if type=search: search.searchAll(query) → ai.analyzeResult (score≥60 为相关)
  store.filterNewItems(items, keyword_id) → [新条目]
  ai.analyze items (并发, limit=15)
  store.saveArticles(all_15_records)  ← 相关的有摘要+score, 不相关的score=0
```
...
title        TEXT NOT NULL
url          TEXT NOT NULL
source       TEXT NOT NULL  -- 'blog'|'google-news'|'hackernews'
snippet      TEXT
summary      TEXT
...
- `get_new_urls(p_keyword_id TEXT, p_urls TEXT[])` → TABLE(url TEXT)
  规避 PostgREST GET 请求 URL 长度限制（尤其 Google News 长 URL），用于 filterNewItems
...
- 边界条件：
  ...
  - filterNewItems URL 超长（Google News）：通过 RPC 函数规避
