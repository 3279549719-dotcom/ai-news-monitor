...
| 🎓 Loop 三层拆解讲解器（决策支持 HTML） | [docs/loop-explainer.html](docs/loop-explainer.html) | 理解 harness loop 三层含义 + 自主交付就绪度诊断（浏览器打开） |
| 📋 REQ：Loop 工程（自主交付协议，Draft） | [docs/REQ-Loop工程-自主交付协议.md](docs/REQ-Loop工程-自主交付协议.md) | 自主交付协议候选 + 验收（待用户拍板实施范围） |
...
| 每日摘要邮件 | `src/email.js` | 管线 run() 末尾 SMTP 推送（buildDigestText/buildSubject/sendDailyDigest） |
